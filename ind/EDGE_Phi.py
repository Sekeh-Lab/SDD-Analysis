...
def calc_dvac(XW, YV, N, agent_type = "pedestrian"):
    ...
    if velocity != 0:
        ...
        acceleration = 1-np.exp(-(12000*acceleration)**2)
    else:
        acceleration = 0
...
def mi_t_edge(data, agent_type = "", phi_type = "", Tp = 25,  hashing="p-stable", ped_i = "NA", ped_j ="NA"):
...
#Determines the equation used for phi calculation being used when determining MI
...
def preprocess_EDGE(i, j, element_x, element_y, csv, frame_list, phi_type):
    ...
    if (j != i) and (element_x == 0 or element_y == 0):
        ...
        if len(check) > 0:
            ...
            if traj_data_i.iloc[0][4] == "pedestrian" or traj_data_i.iloc[0][4] == "bicycle":
                label_i = "Pedestrian"
            elif traj_data_i.iloc[0][4] == "car" or traj_data_i.iloc[0][4] == "truck_bus":
                label_i = "Vehicle"
            else:
                print("Unrecognized Agent Class for: ", traj_data_i.iloc[0][0])

            if traj_data_j.iloc[0][4] == "pedestrian" or traj_data_j.iloc[0][4] == "bicycle":
                label_j = "Pedestrian"
            elif traj_data_j.iloc[0][4] == "car" or traj_data_j.iloc[0][4] == "truck_bus":
                label_j = "Vehicle"
            else:
